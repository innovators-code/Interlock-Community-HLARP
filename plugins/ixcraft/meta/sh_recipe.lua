...
function RECIPE:OnCanSee(ply)
    ...
    if (self.preHooks and self.preHooks["OnCanSee"]) then
        local a, b, c, d, e, f = self.preHooks["OnCanSee"](self, ply)
    ...
    if (self.postHooks and self.postHooks["OnCanSee"]) then
        local a, b, c, d, e, f = self.postHooks["OnCanSee"](self, ply)
...
function RECIPE:OnCanCraft(ply)
    local character = ply:GetCharacter()
    ...
    if (self.preHooks and self.preHooks["OnCanCraft"]) then
        local a, b, c, d, e, f = self.preHooks["OnCanCraft"](self, ply)
    ...
    if (self.postHooks and self.postHooks["OnCanCraft"]) then
        local a, b, c, d, e, f = self.postHooks["OnCanCraft"](self, ply)
...
if ( SERVER ) then
    function RECIPE:OnCraft(ply)
        local bCanCraft, failString, c, d, e, f = self:OnCanCraft(ply)
        ...
        if (self.preHooks and self.preHooks["OnCraft"]) then
            local a, b, c, d, e, f = self.preHooks["OnCraft"](self, ply)
        ...
        for uniqueID, amount in pairs(self.results or {}) do
            ...
            for i = 1, amount do
                if (!inventory:Add(uniqueID)) then
                    ix.item.Spawn(uniqueID, ply)
                end
            end
        end

        if (self.postHooks and self.postHooks["OnCraft"]) then
            local a, b, c, d, e, f = self.postHooks["OnCraft"](self, ply)
