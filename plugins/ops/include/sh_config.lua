...
ix.ops.eventManager.config.CategoryIcons = {
...
ix.ops.eventManager.config.TagColours = {
...
ix.ops.eventManager.config.Events = {
    ["empty"] = {
        Cat = "hidden",
        Prop = {},
        NeedUID = false,
    ...
    ["cineintro"] = {
        ...
        Clientside = false,
        Do = function(prop, uid)
            ix.ops.cinematicIntro(prop["title"])
        end
    },
    ["screenfade"] = {
        Cat = "ui",
    ...
    ["text"] = {
        ...
        Clientside = true,
        Do = function(prop, uid)
            local text = vgui.Create("ixFadeText")
    ...
    ["textarray"] = {
        ...
        Do = function(prop, uid)
            ...
            timer.Create("ixOpsEMTextArray"..math.random(1,100000), prop["array_delay"], max, function()
                local text = vgui.Create("ixFadeText")
    ...
    ["soundplay"] = {
        ...
        Clientside = true,
        Do = function(prop, uid)
            if prop["cponly"] and not LocalPlayer():IsCombine() then
                return
            end
    ...
    ["urlmusic_play"] = {
        ...
        Do = function(prop, uid)
            ...
            net.Start("ixMusicPlayer")
                net.WriteString(tostring(prop["url"]))
            net.Broadcast()

            net.Start("ixMusicPlayerSetVolume")
                net.WriteString(tostring(prop["volume"]))
            net.Broadcast()
        end
    },
    ["urlmusic_stop"] = {
    ...
    ["ammobox_spawn"] = {
        ...
        Do = function(prop, uid)
            ...
            local ent = ents.Create("ix_hl2rp_eventammobox")
            ent:SetPos(prop["pos"])
            ent:SetAngles(Angle(prop["ang"].x, prop["ang"].y, prop["ang"].z))
            ent:Spawn()
            ent:Activate()
    ...
    ["setcookie"] = {
        ...
        Clientside = true,
        Do = function(prop, uid)
            cookie.Set("ix_em_"..prop["name"], prop["value"])
        end
    },
    ["npc_spawn"] = {
        Cat = "npc",
    ...
    ["createscene"] = {
        ...
        Clientside = true,
        Do = function(prop, uid)
            if not ix.ops.eventManager.scenes then
                return
            end
            ...
            ix.ops.eventManager.scenes[uid] = {prop}
        end
    },
    ["playscene"] = {
        Cat = "scene",
