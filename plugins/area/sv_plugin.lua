...
ix.log.AddType("areaAdd", function(ply, name)
    return string.format("%s has added area \"%s\".", ply:Name(), tostring(name))
end)

ix.log.AddType("areaRemove", function(ply, name)
    return string.format("%s has removed area \"%s\".", ply:Name(), tostring(name))
end)
