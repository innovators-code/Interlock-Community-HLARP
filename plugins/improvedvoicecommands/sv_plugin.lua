...
function Schema:PlayerMessageSend(speaker, chatType, text, anonymous, receivers, rawText)
    ...
    if ( Schema.voices.chatTypes[chatType] or Schema.voices.radioChatTypes[chatType] or chatType == "dispatch" or chatType == "DispatchRadio" ) then
        ...
        for k, v in pairs(class) do
            ...
            for k2, v2 in ipairs(texts) do
                ...
                if ( k2 == #texts ) then
                    ...
                    if ( speaker:IsCombine() and not isGlobal ) then
                        speaker.bTypingBeep = nil
                        --table.insert(sounds, "NPC_MetroPolice.Radio.Off")
                    end

                    if ( Schema.voices.radioChatTypes[chatType] ) then
                    ...
                    elseif ( chatType == "dispatch" or chatType == "DispatchRadio" ) then
                        if ( chatType == "DispatchRadio" ) then
                            volume = ix.config.Get("radioVCVolume", 60)
                            if ( v3:IsCombine() ) then
                                if ( ix.config.Get("radioVCClientOnly", false) == true ) then
                                    netstream.Start(receivers, "PlayQueuedSound", nil, sounds, nil, nil, volume)
                                else
                                    for k3, v3 in pairs(receivers) do
        ...
        if ( speaker:IsCombine() ) then
            if not ( Schema.voices.radioChatTypes[chatType] ) then
    ...
    if ( chatType:find("radio_eavesdrop") ) then
