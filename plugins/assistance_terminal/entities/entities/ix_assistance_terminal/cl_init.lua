...
function ENT:Draw( )
    ...
    cam.Start3D2D( self:GetPos( ) + self:GetForward( ) * 10 + self:GetRight( ) * -16 + self:GetUp( ) * 10, angle, 0.0364 )
        ...
        draw.SimpleText( "ASSISTANCE TERMINAL", "CombineFont35", 248, 20, Color( 255, 255, 225, 50 + math.abs( math.sin( CurTime( ) ) ) * 300 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
        surface.SetDrawColor( 44, 44, 44, 255 )
        
        surface.SetDrawColor( 0, 0, 0, 200 )
        surface.DrawRect( 0, 452, 496, 50 )
        draw.SimpleText( area, "CombineFont20", 248, 477, Color( 255, 255, 225, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
        
        if ( self:GetNetVar( "alarm" ) ) then
            surface.SetDrawColor( 255, 0, 0, 255 )
            surface.DrawRect( 0, 80, 496, 120 )
            draw.SimpleText( "OFFICERS HAVE BEEN DISPATCHED", "CombineFont30", 248, 105, Color( 0, 25, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
            draw.SimpleText( "REQUESTER: #" .. self:GetNetVar( "requester", 0 ), "CombineFont30", 248, 140, Color( 255, 255, 0, 50 + math.abs( math.sin( CurTime( ) ) ) * 300 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
            draw.SimpleText( "REMAIN IN PLACE", "CombineFont30", 248, 175, Color( 255, 255, 0, 50 + math.abs( math.sin( CurTime( ) ) ) * 300 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
        else
            ...
            draw.SimpleText( "INSERT ID CARD TO", "CombineFont30", 248, 375, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
            draw.SimpleText( "REQUEST ASSISTANCE", "CombineFont30", 248, 405, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
        end
