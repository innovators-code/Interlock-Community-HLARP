...
if (SERVER) then
   ...
   function PLUGIN:PlayerHurt(ply, attacker, health, damage)
      local attack = attacker:IsPlayer() and attacker:Nick() or attacker:GetClass()
      ix.log.AddRaw("Player: " .. ply:Nick() .. " has taken " .. math.Round(damage, 1) .. " damage from " .. attack .. " | Leaving them at " .. health)
   end

   function PLUGIN:PlayerDeath(ply, wep, attacker)
      local attack = attacker:IsPlayer() and attacker:Nick() or attacker:GetClass()
      ix.log.AddRaw("Player: " .. attack .. " has killed " .. ply:Nick() .. " with " .. wep:GetClass())
   end
