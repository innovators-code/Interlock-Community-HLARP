...
function PLUGIN:CalcView(ply, origin, angles, fov)
    ...
    if not ( IsValid(ply) or ply:GetCharachter() ) then return end
    if ( hidden ) and ( ply:IsDispatch() and ply:GetCharacter():GetClass() == CLASS_G_2_SCANNER ) then
...
function PLUGIN:PreDrawOpaqueRenderables()
    ...
    if ( IsValid(self.lastViewEntity) and not self.lastViewEntity == viewEntity ) and ( ply:IsDispatch() and ply:GetCharacter():GetClass() == CLASS_G_2_SCANNER ) then
        self.lastViewEntity:SetNoDraw(false)
        self.lastViewEntity = nil
        ply:EmitSound(CLICK, 50, 120)
    end

    if ( IsValid(viewEntity) and viewEntity:GetClass():find("scanner") ) and ( ply:IsDispatch() and ply:GetCharacter():GetClass() == CLASS_G_2_SCANNER ) then
...
function PLUGIN:HUDPaint()
    ...
    if ( hidden ) and ( ply:IsDispatch() and ply:GetCharacter():GetClass() == CLASS_G_2_SCANNER ) then
        ...
        local zone = ply:GetArea() or "unknown"
        ...
        draw.SimpleText("ID  ("..ply:Name()..")", "ixScannerFont", x + 8, y + 40, color_white)
        ...
        if ( IsValid(self.lastViewEntity) ) then
            ...
            data.endpos = data.start + ply:GetAimVector() * 500
...
function PLUGIN:PlayerBindPress(ply, bind, pressed)
    ...
    if ( bind:find("attack") and pressed and hidden and IsValid(self.lastViewEntity) ) then
        self:takePicture()
        return true
    elseif ( bind:find("attack2") and pressed and hidden and IsValid(self.lastViewEntity) ) then
        self:takePicture()
        return true
    end
end