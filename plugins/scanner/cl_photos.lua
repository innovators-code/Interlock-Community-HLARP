...
concommand.Add("ix_scanner_photocache", function()
    if ( LocalPlayer():IsCombine() or LocalPlayer():IsDispatch() or LocalPlayer():IsCA() ) then
        ...
        for k, v in ipairs(PHOTO_CACHE) do
            local button = frame.list:Add("ixMenuButton")
            button:SetTall(30)
            button:Dock(TOP)
            button:DockMargin(4, 4, 4, 0)
            button:SetContentAlignment(4)
            button:SetText(os.date("%X - %d/%m/%Y", v.time))
            button:SetFont("InterlockFont25-Light")
        ...
        LocalPlayer():Notify("You do not have access to the photo cache.")
    end
end)