...
--[[
    Packet size is 64kb, where 256 bytes are reserved for hook name and service information.
    There is no subdivision into large messages, because the formation of the 1st packet is instantaneous.
    The data is serialised using pon.
    Data is compressed if its size is greater than the minimum "effective compression" length.
    The maximum message size in the code is limited by the number of packets, this is about 16 mb.

    Transmission speed is determined by net_maxfragments being called 20 times per second. It doesn't depend on the tickrate.
    The default is 1260, which is about 24kb/s.    You can go up to 1792 and get 35 kb/s. net_splitrate has no effect.
    net_splitpacket_maxrate determines how many packets to split, but this only stops the server at lower values.
--]]
