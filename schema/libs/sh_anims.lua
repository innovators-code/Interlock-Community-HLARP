...
function ix.anim.GetModelClass(model)
    if ( istable(model) ) then return "citizen_male" end
    model = string.lower(model)
    local class = translations[model]

    if (!class and ( string.find(model, "/player") or string.find(model, "pm_") or string.find(model, "_pm") or string.find(model, "/pm") )) then
        return "player"
    end
