...
if (SERVER) then
    ...
    function ix.log.AddRaw(logString, bNoSave)
        ...
        MsgC(ix.config.Get("color"), "[Helix Logs] ", ix.log.color[FLAG_DEV], logString .. "\n")
    ...
    function ix.log.Add(client, logType, ...)
        ...
        MsgC(ix.config.Get("color"), "[LOG] ", ix.log.color[FLAG_DEV], logString .. "\n")
    ...
    net.Receive("ixLogStream", function(length)
        ...
        if (isstring(logString) and isnumber(flag)) then
            MsgC(ix.config.Get("color"), "[Helix Logs] ", ix.log.color[flag], logString .. "\n")
        end
    end)
end
