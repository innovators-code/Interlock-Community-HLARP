...
restricted to certain usergroups using a [CAMI](https://github.com/glua/CAMI)-compliant adminOnly mod.
]]
-- @module ix.command
...
-- @field[type=boolean,opt=false] adminOnly Provides an additional check to see if the user is an adminOnly before running.
-- @field[type=boolean,opt=false] superAdminOnly Provides an additional check to see if the user is a superAdminOnly before running.
    ...
    if (!data.OnCheckAccess) then
        ...
        if (!CAMI.GetPrivilege(privilege)) then
            CAMI.RegisterPrivilege({
                Name = privilege,
                MinAccess = data.superadmin and "superadmin" or (data.superadmin and "admin" or "user"),
