...
function SWEP:PrimaryAttack()
    ...
    if (ix.plugin.Get("stamina")) then
        ...
        if (staminaUse > 0) then
            local value = self:GetOwner():GetLocalVar("attribute_stamina", 0) - staminaUse
    ...
    if ( SERVER ) then
        self:GetOwner():EmitSound("WeaponFrag.Throw")
    end
                ...
                if (SERVER and trace.Hit) then
                    ...
                    if (IsValid(entity)) then
                        ...
                        if ( entity:IsPlayer() ) then
                            local direction
                            if ( self:GetOwner():GetModel():find("combine_heavy_trooper") ) then
                                entity:EmitSound("physics/body/body_medium_break"..math.random(2,4)..".wav", 80, nil, 2)
                                direction = self:GetOwner():GetAimVector() * 500
                            else
                                direction = self:GetOwner():GetAimVector() * 200
                            end
                            direction.z = 0
